...
Changes Made:
    ...
    - Updates target_branch references from 'main' to the release branch
    - Updates pipelineRef revision to the release branch
    - Adds version suffix to output image names (e.g., -1-8)
    - Renames pipeline names with version suffix
    - Renames pipeline files to match new naming convention

    Makefile:
    - Updates VERSION to x.y.0 (e.g., 1.8.0)
    - Updates CHANNELS to include the new release channel

    Config Manifests:
    - Updates version in CSV base (config/manifests/bases/)
    - Updates documentation URL to new version

    Dockerfiles:
    - Updates version labels (e.g., v1.8.0)
    - Updates CPE references
    - Sets release label to '1'
...
update_config_manifests() {
    ...
    if [[ -f "$csv_base" ]]; then
        ...
        # Update version field (e.g., version: 1.5.1 -> version: 1.8.0)
        run_sed "s/^  version: [0-9]\+\.[0-9]\+\.[0-9]\+/  version: ${VERSION}.0/g" "$csv_base"
